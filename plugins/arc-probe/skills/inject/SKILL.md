...
4. Verify the probe is connected by calling `probe_status`. This should return process info including PID, module bases, and driver status.

5. If `probe_status` fails:
   - Check that the target process is still running
   - Check that port 9999 is not in use by another instance: `netstat -ano | findstr :9999`
...
- If port 9999 is already in use, suggest setting PROBE_PORT to a different value
